(function ($) {
				...
				if ($.isArray(items)) {
					...
					$.each(items, function (i, opt) {
						if (typeof valuesHash[opt.id] === 'undefined') {
							$('<option value="' + opt.id + '">' + opt.label + '</option>').appendTo(select);
						}
					});
				} else {
    ...
    function processValue(e) {
        ...
        (e.which && (
			...
			//show loading
			input.css({
				'background': 'transparent url("' + loadingImg + '") no-repeat right 3px',
				'background-size': '10%'
			})
			//input.get(0).css.background = 'transparent url("' + loadingImg + '") no-repeat right 3px';
			//input.get(0).style.background-size = '10%';
